...
export interface IntegritySelection {
  years: number[]
  rank: number | null
}
...
export const resolveIntegritySelection = (
  args: string[],
): IntegritySelection => {
  ...
  if (filteredArgs.length === 0)
    throw new Error(
      'Integrity selection requires at least one selector: --all, --year=1945,1946, or --decade=1940.',
    )
